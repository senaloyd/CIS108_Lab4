...
bool isPersonAllowed (std::string person)
{
	...
	for (int x = 0; x < (sizeof (ALLOW_ACCESS) / sizeof (char*)); x++)
	{
		...
		{
			is_allowed = true;  //1st Error C4553
			break;
		}
	}
...
void ssDoorLockEventHandler (DoorLock::DoorLockEvent event, std::string pin_code) //changed int to std::string
{
	...
	switch (event)
	{
		...
		{
			if (allowed_person_inside)  //2nd Error C2061 and C2059
			{
		...
		{
			if (pin_code == SECURE_PIN_CODE)  //4th Error C2678  ...  std::string SECURE_PIN_CODE
			{
				access_granted = true;
				DoorLock::unlockDoor ();
			}
			else (pin_code == SECURE_PIN_CODE); //Come back to this line to look at issues, totally cannot remember what I did here.
			{
				allowed_person_inside = false;
			}
		} break;
...
void ssVideoDoorbellEventHandler (VideoDoorbell::VideoDoorbellEvent event, std::string person_at_door)
{
	...
	switch (event)
	{
		...
		{
			MobileAppInterface::notifyFrontDoorActivity(false); //3rd Error (forgot to write code) needed a ;
		} break;
	}
}
